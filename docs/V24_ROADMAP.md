...
Validation note: the initial v24.0.1 gate exposed a signed-Amount regression where the generic import sanitizer removed a legitimate leading minus before bank direction classification. The repair is intentionally scoped to the bank-money parser, preserves the global formula-injection guard, and adds explicit formula-like-input regression coverage. A temporary CI bootstrap may write only the parser/test repair after that repaired source passes the complete suite; the normal release workflow remains the final merge gate.
