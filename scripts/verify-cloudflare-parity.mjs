...
async function runLiveChecks(checks) {
  ...
  assert(checks, 'Index references app.js v24.0.42', index.text.includes('app.js?v=24.0.42'));
  ...
  assert(checks, 'Index references sw-bridge.js v24.0.42', index.text.includes('sw-bridge.js?v=24.0.42'));

  const sw = await fetchText(`${appOrigin}/service-worker.js?verify=${Date.now()}`);
  assert(checks, 'Service worker loads', sw.ok, `${sw.status}`);
  assert(checks, 'Service worker version 24.0.42', sw.text.includes("SW_VERSION = '24.0.42'"));
  ...
  const overlay = await fetchText(`${appOrigin}/midwest-stack-authority.js?v=24.0.42`);
  ...
  const bridge = await fetchText(`${appOrigin}/sw-bridge.js?v=24.0.42`);
  assert(checks, 'SW bridge loads', bridge.ok, `${bridge.status}`);
  assert(checks, 'SW bridge imports modern-shell.js v24.0.42', bridge.text.includes("modern-shell.js?v=24.0.42"));

  const shell = await fetchText(`${appOrigin}/modern-shell.js?v=24.0.42`);
  ...
  assert(checks, 'Service worker precaches modern-shell.js v24.0.42', sw.text.includes('modern-shell.js?v=24.0.42'));

  const manifest = await fetchJson(`${appOrigin}/manifest.json?v=24.0.42`);
  assert(checks, 'Manifest loads', manifest.ok, `${manifest.status}`);
  assert(checks, 'Manifest name v24.0.42', manifest.json && manifest.json.name === EXPECTED.manifestName, manifest.json && manifest.json.name);
