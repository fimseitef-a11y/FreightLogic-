...
const EXPECTED = {
  serviceWorkerVersion: "24.0.43",
  manifestName: "FreightLogic v24.0.43",
  workerVersion: "28",
  overlayScript: "midwest-stack-authority.js?v=24.0.43"
};
...
async function runLiveChecks(checks) {
  ...
  assert(checks, 'Index references app.js v24.0.43', index.text.includes('app.js?v=24.0.43'));
  ...
  assert(checks, 'Index references sw-bridge.js v24.0.43', index.text.includes('sw-bridge.js?v=24.0.43'));

  const sw = await fetchText(`${appOrigin}/service-worker.js?verify=${Date.now()}`);
  assert(checks, 'Service worker loads', sw.ok, `${sw.status}`);
  assert(checks, 'Service worker version 24.0.43', sw.text.includes("SW_VERSION = '24.0.43'"));
  ...
  const overlay = await fetchText(`${appOrigin}/midwest-stack-authority.js?v=24.0.43`);
  ...
  const bridge = await fetchText(`${appOrigin}/sw-bridge.js?v=24.0.43`);
  assert(checks, 'SW bridge loads', bridge.ok, `${bridge.status}`);
  assert(checks, 'SW bridge imports modern-shell.js v24.0.43', bridge.text.includes("modern-shell.js?v=24.0.43"));

  const shell = await fetchText(`${appOrigin}/modern-shell.js?v=24.0.43`);
  ...
  assert(checks, 'Service worker precaches modern-shell.js v24.0.43', sw.text.includes('modern-shell.js?v=24.0.43'));

  const manifest = await fetchJson(`${appOrigin}/manifest.json?v=24.0.43`);
  assert(checks, 'Manifest loads', manifest.ok, `${manifest.status}`);
  assert(checks, 'Manifest name v24.0.43', manifest.json && manifest.json.name === EXPECTED.manifestName, manifest.json && manifest.json.name);
  ...
  // malformed key here means Web Push cannot work for anyone.
  const pushKey = await fetchJson(`${workerOrigin}/push/key`);
  const pk = pushKey.json && typeof pushKey.json.publicKey === 'string' ? pushKey.json.publicKey : '';
  let pkBytes = null;
  try { pkBytes = /^[A-Za-z0-9_-]+$/.test(pk) ? Buffer.from(pk.replace(/-/g, '+').replace(/_/g, '/'), 'base64') : null; } catch { pkBytes = null; }
  assert(checks, 'Worker /push/key serves a 65-byte uncompressed P-256 VAPID key',
    pushKey.status === 200 && !!pkBytes && pkBytes.length === 65 && pkBytes[0] === 4,
    `${pushKey.status} ${pk ? pk.length + ' chars' : JSON.stringify(pushKey.json)}`);
