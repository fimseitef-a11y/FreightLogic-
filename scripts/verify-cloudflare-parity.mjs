...
const EXPECTED = {
  serviceWorkerVersion: "24.0.6",
  manifestName: "FreightLogic v24.0.6",
  workerVersion: "14",
  overlayScript: "midwest-stack-authority.js?v=24.0.6"
};
...
async function runLiveChecks(checks) {
  ...
  assert(checks, 'Index references app.js v24.0.6', index.text.includes('app.js?v=24.0.6'));
  assert(checks, 'Index references voice-load.js v24.0.6', index.text.includes('voice-load.js?v=24.0.6'));
  assert(checks, 'Index references sw-bridge.js v24.0.6', index.text.includes('sw-bridge.js?v=24.0.6'));

  const sw = await fetchText(`${appOrigin}/service-worker.js?verify=${Date.now()}`);
  assert(checks, 'Service worker loads', sw.ok, `${sw.status}`);
  assert(checks, 'Service worker version 24.0.6', sw.text.includes("SW_VERSION = '24.0.6'"));
  ...
  const overlay = await fetchText(`${appOrigin}/midwest-stack-authority.js?v=24.0.6`);
  ...
  const manifest = await fetchJson(`${appOrigin}/manifest.json?v=24.0.6`);
  assert(checks, 'Manifest loads', manifest.ok, `${manifest.status}`);
  assert(checks, 'Manifest name v24.0.6', manifest.json && manifest.json.name === EXPECTED.manifestName, manifest.json && manifest.json.name);
