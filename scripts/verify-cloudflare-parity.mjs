...
const EXPECTED = {
  serviceWorkerVersion: "24.1.0",
  manifestName: "FreightLogic v24.1.0",
  workerVersion: "13",
  overlayScript: "midwest-stack-authority.js?v=24.1.0"
};
...
async function main() {
  ...
  assert(checks, 'Index references app.js v24.1.0', index.text.includes('app.js?v=24.1.0'));
  assert(checks, 'Index references voice-load.js v24.1.0', index.text.includes('voice-load.js?v=24.1.0'));
  assert(checks, 'Index references sw-bridge.js v24.1.0', index.text.includes('sw-bridge.js?v=24.1.0'));

  const sw = await fetchText(`${pagesOrigin}/service-worker.js?verify=${Date.now()}`);
  assert(checks, 'Service worker loads', sw.ok, `${sw.status}`);
  assert(checks, 'Service worker version 24.1.0', sw.text.includes("SW_VERSION = '24.1.0'"));
  ...
  const overlay = await fetchText(`${pagesOrigin}/midwest-stack-authority.js?v=24.1.0`);
  ...
  const manifest = await fetchJson(`${pagesOrigin}/manifest.json?v=24.1.0`);
  assert(checks, 'Manifest loads', manifest.ok, `${manifest.status}`);
  assert(checks, 'Manifest name v24.1.0', manifest.json && manifest.json.name === EXPECTED.manifestName, manifest.json && manifest.json.name);
