...
const EXPECTED = {
  serviceWorkerVersion: "24.0.22",
  manifestName: "FreightLogic v24.0.22",
  workerVersion: "21",
  overlayScript: "midwest-stack-authority.js?v=24.0.22"
};
...
async function runLiveChecks(checks) {
  ...
  assert(checks, 'Service worker version 24.0.22', sw.text.includes("SW_VERSION = '24.0.22'"));
  ...
  const overlay = await fetchText(`${appOrigin}/midwest-stack-authority.js?v=24.0.22`);
  ...
  // v24.0.22: the structural navigation adapter is a release-bound asset and was
  ...
  const bridge = await fetchText(`${appOrigin}/sw-bridge.js?v=24.0.22`);
  assert(checks, 'SW bridge loads', bridge.ok, `${bridge.status}`);
  assert(checks, 'SW bridge imports modern-shell.js v24.0.22', bridge.text.includes("modern-shell.js?v=24.0.22"));

  const shell = await fetchText(`${appOrigin}/modern-shell.js?v=24.0.22`);
  ...
  assert(checks, 'Service worker precaches modern-shell.js v24.0.22', sw.text.includes('modern-shell.js?v=24.0.22'));

  const manifest = await fetchJson(`${appOrigin}/manifest.json?v=24.0.22`);
  assert(checks, 'Manifest loads', manifest.ok, `${manifest.status}`);
  assert(checks, 'Manifest name v24.0.22', manifest.json && manifest.json.name === EXPECTED.manifestName, manifest.json && manifest.json.name);
