...
const EXPECTED = {
  serviceWorkerVersion: "24.0.18",
  manifestName: "FreightLogic v24.0.18",
  workerVersion: "20",
  overlayScript: "midwest-stack-authority.js?v=24.0.18"
};
...
async function runLiveChecks(checks) {
  ...
  assert(checks, 'Index references app.js v24.0.18', index.text.includes('app.js?v=24.0.18'));
  // Voice Load was removed completely by operator decision (Issue #230,
  // v24.0.18). The positive reference assertion is replaced by its absence:
  ...
  assert(checks, 'Index references sw-bridge.js v24.0.18', index.text.includes('sw-bridge.js?v=24.0.18'));

  const sw = await fetchText(`${appOrigin}/service-worker.js?verify=${Date.now()}`);
  assert(checks, 'Service worker loads', sw.ok, `${sw.status}`);
  assert(checks, 'Service worker version 24.0.18', sw.text.includes("SW_VERSION = '24.0.18'"));
  ...
  const overlay = await fetchText(`${appOrigin}/midwest-stack-authority.js?v=24.0.18`);
  ...
  // v24.0.18: the structural navigation adapter is a release-bound asset and was
  ...
  const bridge = await fetchText(`${appOrigin}/sw-bridge.js?v=24.0.18`);
  assert(checks, 'SW bridge loads', bridge.ok, `${bridge.status}`);
  assert(checks, 'SW bridge imports modern-shell.js v24.0.18', bridge.text.includes("modern-shell.js?v=24.0.18"));

  const shell = await fetchText(`${appOrigin}/modern-shell.js?v=24.0.18`);
  ...
  assert(checks, 'Service worker precaches modern-shell.js v24.0.18', sw.text.includes('modern-shell.js?v=24.0.18'));

  const manifest = await fetchJson(`${appOrigin}/manifest.json?v=24.0.18`);
  assert(checks, 'Manifest loads', manifest.ok, `${manifest.status}`);
  assert(checks, 'Manifest name v24.0.18', manifest.json && manifest.json.name === EXPECTED.manifestName, manifest.json && manifest.json.name);
