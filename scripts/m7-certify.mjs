...
for (const [g, cmd] of [
  ...
  ['Live invite/claim contract (B7)', 'GitHub workflow — Verify Authenticated Worker (runs scripts/verify-live-invite-claim.mjs after every dispatched Worker deploy). OBSERVED PASS run 35048574588 @da0667c; re-observe after any Worker redeploy. Do NOT run it twice inside one hour — /claim is 10/hr per IP and the second result reads UNOBSERVED, which is not a pass and not a failure'],
