...
test('[FIELD CERT / NEW] FC-01 runner loads and exposes exactly A1-A12 as OPEN', async () => {
  ...
  try {
    ...
    eq(gates.every(g => g.status === 'OPEN'), true,
      'no physical-device gate may be pre-certified or auto-passed on page load');
  } finally {
    await app.close();
  }
});
