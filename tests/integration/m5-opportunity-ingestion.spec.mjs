...
test('[M5-05] an operator-confirmed expected revenue is allowed regardless of semantic', async () => {
  ...
  ok(r.provenance.operatorConfirmedAt !== null, 'confirmation timestamp set');
});
...
test('[M5-08] mileage semantics stay distinct — a displayed total is not loaded miles', async () => {
  ...
  eq(r.loadedMi, 480, 'the number is kept');
  eq(r.mileageSemantic, 'DISPLAYED_TOTAL_MILES', 'but labelled as a displayed total, not loaded miles');
});
