...
// this codebase:
//
//   ZTO-01..03 (#231 Phase C) the driver app carries no admin UI, no admin
//              API call, and boot deletes any admin credential an earlier
//              build stored — delete-only, never read or promoted.
//   ZTO-05     a legacy admin ciphertext is absent from the export payload
//              AND from the checksum input (X-05 class).
...
// ── #231 Phase C: the driver app carries no admin surface ────────────────────
//
// ZTO-01..04, ZTO-11 and ZTO-12 used to drive the in-app admin panel: verify
// before persist, ciphertext at rest, PIN unlock, the Drivers list, and invite.
// That surface moved to the separate-origin Admin Console (admin-console/,
// covered by tests/integration/admin-console.spec.mjs and live-verified by
// verify-admin-console.yml). What the driver app must now prove is ABSENCE,
// plus a delete-only cleanup of credentials earlier builds stored.

test('[ZTO-01] the driver app renders no admin control and exposes no admin API path', async () => {
  const app = await openApp();
  try {
    const log = [];
    await routeWorker(app.page, {}, log);
    await app.page.goto(`${app.baseUrl}/index.html#insights`);
    await app.page.waitForTimeout(400);
    const out = await app.page.evaluate(() => ({
      ids: ['btnAdminToggle', 'adminPanel', 'adminToken', 'adminUserList', 'btnAdminCreate',
            'btnAdminSaveAccess', 'adminDriversBlock'].filter(id => document.getElementById(id)),
      text: document.body.innerText,
      scripts: [...document.scripts].map(s => s.src).filter(Boolean),
      fns: ['cloudAdminSaveAccess', 'cloudAdminInviteDriver', 'cloudAdminLoadUsers',
            'cloudAdminResolveToken', 'renderAdminAccessState'].filter(n => n in (window.__FL_TESTS || {})),
    }));
    eq(out.ids.length, 0, `admin elements still in the driver DOM: ${out.ids.join(',')}`);
    ok(!/Add \/ Manage Drivers|Admin access/i.test(out.text), 'no admin wording may remain on the driver surface');
    ok(!out.scripts.some(u => /admin-driver-ui\.js/.test(u)), 'admin-driver-ui.js must not load');
    eq(out.fns.length, 0, `admin functions still reachable: ${out.fns.join(',')}`);
    eq(log.filter(e => e.path.startsWith('/admin')).length, 0, 'the driver app must never call an /admin endpoint');
  } finally { await app.close(); }
});

test('[ZTO-02] boot deletes a legacy admin credential from settings, session and local storage', async () => {
  const app = await openApp();
  try {
    await app.page.evaluate(async () => {
      await window.__FL_TESTS.setSetting('cloudAdminTokenEnc', { encrypted: 'LEGACY-CIPHERTEXT', iv: 'IV', salt: 'SALT' });
      sessionStorage.setItem('fl_admin_tok', 'legacy-session-admin');
      localStorage.setItem('fl_admin_tok', 'legacy-local-admin');
    });
    await app.page.reload({ waitUntil: 'load' });
    await waitForAppReady(app.page);
    // The purge runs during boot right after initDB; poll (Node-side, awaiting
    // each evaluate) rather than race it.
    let out = null;
    for (let i = 0; i < 50; i++) {
      out = await app.page.evaluate(async () => {
        const rows = await window.__FL_TESTS.dumpStore('settings');
        return {
          stored: rows.some(r => r.key === 'cloudAdminTokenEnc' && r.value != null),
          session: sessionStorage.getItem('fl_admin_tok'),
          local: localStorage.getItem('fl_admin_tok'),
        };
      });
      if (!out.stored && out.session == null && out.local == null) break;
      await app.page.waitForTimeout(100);
    }
    eq(out.stored, false, 'the encrypted admin credential must be deleted at boot');
    eq(out.session, null, 'the session admin token must be deleted');
    eq(out.local, null, 'a legacy localStorage admin token must be deleted');
  } finally { await app.close(); }
});

test('[ZTO-03] the cleanup is delete-only and leaves driver credentials alone', async () => {
  const app = await openApp();
  try {
    const out = await app.page.evaluate(async () => {
      await window.__FL_TESTS.setSetting('cloudBackupToken', 'flk_' + 'b'.repeat(32));
      await window.__FL_TESTS.setSetting('cloudAdminTokenEnc', { encrypted: 'X', iv: 'Y', salt: 'Z' });
      const ran = await window.__FL_TESTS.purgeLegacyAdminCredential();
      const rows = await window.__FL_TESTS.dumpStore('settings');
      return {
        ran,
        admin: rows.find(r => r.key === 'cloudAdminTokenEnc') || null,
        driver: (rows.find(r => r.key === 'cloudBackupToken') || {}).value || null,
        cached: await window.__FL_TESTS.getSetting('cloudAdminTokenEnc', 'ABSENT'),
      };
    });
    eq(out.ran, true, 'the cleanup must report it ran');
    eq(out.admin, null, 'the admin credential row must be gone, not nulled');
    eq(out.cached, 'ABSENT', 'the settings cache must not keep serving the deleted credential');
    ok(/^flk_b{32}$/.test(out.driver || ''), 'the driver backup token must survive the admin cleanup');
  } finally { await app.close(); }
});
