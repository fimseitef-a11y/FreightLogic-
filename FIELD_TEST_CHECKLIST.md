...
Current runtime synchronization point: **production serves FreightLogic v24.0.25 / IndexedDB v16 / Worker v21, and BOTH generations are OBSERVED** — re-dispatched live parity `35542846195` (job `106163516058`) and production service worker `35542851411` (job `106163528152`), both `VERDICT: PASS` on `266d74e`. The manifest reads `FreightLogic v24.0.25`, Worker `/health` reports v21, all 22 declared runtime assets load with none served as HTML, 20 repository-only paths stay non-public, and the precache is `freightlogic-24.0.25` with exactly one generation cache. **A13's Worker-v21 prerequisite is therefore discharged** — that row is OPEN like the rest of the A section rather than BLOCKED. **This line is a lookup, not a record:** re-read `APP_VERSION` and `docs/COMPLETION_RELEASE_CERTIFICATION_ADDENDUM_2026-09-20.md` every session. It has now gone stale twice — by three generations once, and by one generation at v24.0.25 — which is the failure this file exists to prevent: a tester confirming the wrong build and recording a PASS for a candidate that is not the one being certified.
