...
- **Superseded 2026-09-24: production is now v24.0.35 / DB16 / Worker v24** (see the paragraph
  ...
  first read "#231 stays open until that gate is re-dispatched", which was already superseded
  when it was written.*
...
**Production app is v24.0.46 / DB16, DIRECTLY OBSERVED 2026-09-26; the deployed Worker is v28 while source is v30.** PR #388 merged as `3e4fca2` (#386 hotfix). Re-dispatched Production Service Worker `36235827108` PASS and Live Parity `36235825920` with every app-side check at 24.0.46 and exactly one failure, `Worker reports v30 — {"version":"28"}` (#380, unchanged). The first two parity dispatches (`36235604153`, `36235624901`) and SW `36235603089` ran within 45 s of the merge, saw 24.0.45, and are the propagation race, not the evidence.

*Superseded, kept as history:* **Production app was v24.0.45 / DB16, DIRECTLY OBSERVED 2026-09-26; the deployed Worker is v28 while source is v30.** PR #385 merged as `bd32ff0` (the two-output bid, app-only). Re-dispatched Production Service Worker `36234354289` PASS on that SHA. Re-dispatched Live Parity `36234353168` observed every app-side check at 24.0.45 (21 assets, 24 withheld paths), and failed exactly one check: `Worker reports v30 — {"version":"28"}`. Worker v29 (#375, reminders) and v30 (#381, PushWard) are merged but not deployed; the v30 deploy is waiting on the operator setting `PUSHWARD_INTEGRATION_KEY`. That one failure is the known Worker gap, not a v24.0.45 defect. v24.0.44 (`c6b2314`) was never recorded here as observed on its own; v24.0.45 supersedes it.

*Superseded, kept as history:* **Production is v24.0.43 / DB16 / Worker v28, DIRECTLY OBSERVED 2026-09-26.** PR #371 merged as `6a80282` (trip import integrity, app-only). Re-dispatched Live Parity `36208352685` and Production Service Worker `36208353917` PASS on `main` @ `0c582bf`, whose runtime tree is identical to `6a80282` (#369 is docs). Push-triggered parity `36208205933` on `6a80282` raced the Cloudflare deploy and is not the evidence.
...
*Superseded, kept as history:* **Production was v24.0.36 / DB16 / Worker v24, DIRECTLY OBSERVED 2026-09-24.** PR #348 merged as
...
- Refreshed fuel baseline — this lives in `MW.fuelBaseline` (`app.js:5961`,
  `3.55` in that historical release), *not* in the rate-overrides JSON, which has no fuel field.
  The v24.0.26 dated profile supersedes that fallback; derive current defaults from source.
...
**DEPLOYED and OBSERVED LIVE 2026-09-25.** Merged as `0af03d4` (PR #350). Re-dispatched Live Parity
`36093836038` and Production Service Worker `36093837857` PASS on that SHA, observing 24.0.37;
...
### Production observation and remaining #278 scope — 2026-09-21

The core slice is merged and live, with current-generation evidence in the Project Overview
and the 2026-09-21 certification state. The later accepted #278 addenda are **not complete**:
explicit settlement/rate-basis semantics; dynamic/regional fuel provenance; chain and exit
economics; distinct DEACTIVATED/WITHDRAWN outcomes; evidence-driven market clearing; and
replacement of the legacy >250-mile $1.45 decision floor with reconciled contextual policy.
The required independent Claude audit/joint-consensus gate remains open. Existing `V24-B04`
still tests the legacy long-haul rule; green tests do not close that policy requirement.
...
### Historical v24.0.25 deployment observation — 2026-09-20
...
Today surface with no uncaught page errors.** That observes shell availability. It does not independently exercise every More category or
Text Size / Glance control; those interactions have separate regression and device evidence. `scripts/verify-cloudflare-parity.mjs --static-only`
