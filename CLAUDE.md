...
**FreightLogic v24.0.28 / DB16 / Worker v21 is directly observed in production.**
...
**Current production observation of record:** checkpoint `675e6fb0140f0e226219fc52955932703fc07a1c` directly passed Tests `35661894446` (job `106538812253`) at **746/0 across 72 specs**, Live Parity `35661894518` (job `106538813802`), Production Service Worker `35661894391` (job `106538814618`), and CodeQL `35661894363`. The live logs directly observe app/SW/manifest **24.0.28**, Worker **v21**, all **22** declared runtime assets, **20** repository-only paths withheld, `freightlogic-24.0.28` with all 22 assets in precache, five driver tabs + Today after reload, no uncaught page errors, and exactly one app-generation cache.

The test-only F-9 stabilization merged before that checkpoint as `0fc250cb0523bc040049cd25fbf6f12e9a5b376c` (PR #298). It removed two unnecessary asynchronous sleeps from the regression itself; `app.js` remained byte-identical and the negative control still failed as intended. Preserve the earlier 745/1 run as historical test-control evidence, but do not describe it as an unresolved product defect or as a runtime repair.
