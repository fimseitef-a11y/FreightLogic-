...
## Concrete collision fixture

Using the exact current DJB2 loop (`h = (((h << 5) + h) + charCode) >>> 0`, seed 5381), these distinct 24-character strings collide:

```text
JaRWdaC8PladIXNaucQuLJs4
...
1. two synthetic distinct rows engineered to collide under the old 32-bit fingerprint must still import as distinct records under the replacement;
2. keep the original long-provenance re-import idempotency regression that motivated `d9d01a7`;
3. persisted fingerprint length remains below `migratedFrom`'s 120-character clamp.

Preserve the core insight of `d9d01a7`: the persisted token must stay bounded and must be computed from the full untruncated semantic input.